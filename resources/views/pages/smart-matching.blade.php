@auth
    @if(auth()->user()->role === 'student')
        @php
            ...
            $recentInternships = App\Models\Internship::where('is_active', true)
                ->where(function ($q) use ($studentSkillArray) {
        ...
        @if($recentInternships->count() > 0)
            <div class="mb-6 p-4 bg-green-50 rounded-lg">
                <p class="text-sm text-green-700">Based on your skills ({{ $studentSkills ?: 'None specified' }}), we found
                    {{ $recentInternships->count() }} matching internships.</p>
            </div>
        @endif
    @endif
@endauth
    ...
    <div class="max-w-7xl mx-auto">
        <div class="text-center mb-12">
            ...
            <p class="text-xl text-gray-600 max-w-2xl mx-auto">
                Our platform matches students and businesses based on skills and needs — no long hiring process
                required.
            </p>
        </div>
        ...
        @if($recentInternships->count() > 0 || $recentStudents->count() > 0)
                        ...
                        <div class="space-y-4">
                            @foreach($recentInternships as $internship)
                                <div class="border-b border-gray-100 pb-3 last:border-0">
                                    ...
                                    <p class="text-sm text-gray-600">{{ $internship->business->name ?? 'Company' }} •
                                        {{ ucfirst($internship->type) }} • {{ $internship->location }}</p>
                                    <div class="mt-2 flex flex-wrap gap-2">
                                        <span
                                            class="text-xs bg-green-100 text-green-700 px-2 py-1 rounded">{{ $internship->duration }}</span>
                                        <span
                                            class="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">{{ ucfirst($internship->type) }}</span>
                                    </div>
                                    <a href="{{ route('internships.show', $internship) }}"
                                        class="text-blue-600 text-sm mt-2 inline-block font-semibold">Apply Now →</a>
                                </div>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="text-center mt-10">
                <a href="{{ route('internships.index') }}"
                    class="inline-block bg-blue-600 text-white px-8 py-3 rounded-full font-semibold hover:bg-blue-700 transition shadow-md">Browse
                    All Opportunities</a>
            </div>
        @else
            <div class="text-center py-12 bg-white rounded-xl shadow-sm">
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center mt-6">
                    <a href="{{ route('register') }}" class="bg-blue-600 text-white px-6 py-2 rounded-full">Join as
                        Student</a>
                    <a href="{{ route('register.business') }}"
                        class="border border-blue-600 text-blue-600 px-6 py-2 rounded-full">Post an Internship</a>
                </div>
            </div>
        @endif
    </div>
